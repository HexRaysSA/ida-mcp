"""Portable log archives for IDA MCP semantic and agent sessions."""
...
def _select_sessions(
        ...
        for requested in session_files:
            ...
            if not _is_semantic_session(path):
                raise LogArchiveError(
                    f"not an IDA MCP semantic session (schema 1): {requested}"
                )
    ...
    if not unique:
        raise LogArchiveError("no IDA MCP semantic sessions found")
...
def _find_agent_file(recorded_path: str, semantic_session: Path) -> Path | None:
    try:
        ...
        # Benchmark bundles retain the MCP trace below logs/ida-mcp while the
        # corresponding agent transcript is logs/session.jsonl.
...
def _agent_references(path: Path) -> list[_AgentReference]:
    ...
    # that never received or called an IDA MCP tool and therefore cannot
...
def open_log_archive(path: Path) -> Iterator[LogArchiveView]:
    ...
    with tempfile.TemporaryDirectory(prefix="ida-mcp-logs-") as directory:
        yield _extract_log_archive(archive_path, Path(directory).resolve())


def _default_output() -> Path:
    stamp = datetime.now(UTC).astimezone().strftime("%Y%m%d-%H%M%S")
    return Path.cwd() / f"ida-mcp-logs-{stamp}.zip"
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="ida-mcp logs",
        description=(
            "Create a ZIP containing IDA MCP semantic sessions, linked "
    ...
    try:
        ...
        print(f"ida-mcp logs: error: {exc}", file=sys.stderr)
        return 2

    for missing in result.missing_agent_sessions:
        print(
            "ida-mcp logs: warning: linked "
